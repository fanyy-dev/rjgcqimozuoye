...
public class OrdersService {
    ...
    public Page<Orders> search(Page<Orders> page,Orders orders) {
        Page<Orders> resultPage = ordersMapper.selectPage(page, getQueryWrapper(orders));
        for (Orders o : resultPage.getRecords()) {
    ...
    public List<Orders> search(Orders orders) {
        return ordersMapper.selectList(getQueryWrapper(orders));
    }

    //条件构造器
    private QueryWrapper<Orders> getQueryWrapper(Orders orders){
        QueryWrapper<Orders> queryWrapper = new QueryWrapper<>();
        if(null != orders){
            if(!StringUtils.isEmpty(orders.getRemark())){
                queryWrapper.like("remark",String.valueOf(orders.getRemark()));
            }
            if (!StringUtils.isEmpty(orders.getMemberId())) {
                queryWrapper.eq("member_id", String.valueOf(orders.getMemberId()));
            }
            if (!StringUtils.isEmpty(orders.getStatus())) {
                queryWrapper.eq("status", String.valueOf(orders.getStatus()));
            }
    ...
    public Result add(OrderQuery orderQuery) {
        ...
        //判断用户是否存在，不存在则创建
        List<Member> memberList = memberMapper.selectList(new QueryWrapper<Member>().eq("idcard",orderQuery.getIdcard()));

...
public class AdminService {
    ...
    public Page<Admin> search(Page<Admin> page,Admin admin) {
        return adminMapper.selectPage(page,getQueryWrapper(admin));
    }

    public List<Admin> search(Admin admin) {
        return adminMapper.selectList(getQueryWrapper(admin));
    }

    //条件构造器
    private QueryWrapper getQueryWrapper(Admin admin){
        QueryWrapper queryWrapper = new QueryWrapper();
        if(null != admin){
            if(!StringUtils.isEmpty(admin.getUsername())){
                queryWrapper.like("username",String.valueOf(admin.getUsername()));
            }
            if(!StringUtils.isEmpty(admin.getName())){
                queryWrapper.like("name",String.valueOf(admin.getName()));
            }
        }
        return queryWrapper;
    }
    ...
    public Result login(LoginQuery query) {
        List<Admin> list = adminMapper.selectList(new QueryWrapper<Admin>().eq("username", query.getUsername()).eq("password", query.getPassword()));

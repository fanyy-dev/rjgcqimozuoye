...
public class MemberService {
    ...
    public Page<Member> search(Page<Member> page,Member member) {
        return memberMapper.selectPage(page,getQueryWrapper(member));
    }

    public List<Member> search(Member member) {
        return memberMapper.selectList(getQueryWrapper(member));
    }

    //条件构造器
    private QueryWrapper getQueryWrapper(Member member){
        QueryWrapper queryWrapper = new QueryWrapper();
        if(null != member){
            if(!StringUtils.isEmpty(member.getUsername())){
                queryWrapper.like("username",String.valueOf(member.getUsername()));
            }
            if(!StringUtils.isEmpty(member.getPassword())){
                queryWrapper.like("password",String.valueOf(member.getPassword()));
            }
            if(!StringUtils.isEmpty(member.getName())){
                queryWrapper.like("name",String.valueOf(member.getName()));
            }
            if(!StringUtils.isEmpty(member.getPhone())){
                queryWrapper.like("phone",String.valueOf(member.getPhone()));
            }
            if(!StringUtils.isEmpty(member.getIdcard())){
                queryWrapper.like("idcard",String.valueOf(member.getIdcard()));
            }
            if(!StringUtils.isEmpty(member.getHead())){
                queryWrapper.like("head",String.valueOf(member.getHead()));
            }
        }
        return queryWrapper;
    }
    ...
    public Result login(LoginQuery query) {
        List<Member> list = memberMapper.selectList(new QueryWrapper<Member>().eq("username", query.getUsername()).eq("password", query.getPassword()));
    ...
    public Result register(Member member) {
        List<Member> list = memberMapper.selectList(new QueryWrapper<Member>().eq("username", member.getUsername()));
        if(list.size() != 0)return new Result(false, StatusCode.ERROR,"账号已被注册！");

        list = memberMapper.selectList(new QueryWrapper<Member>().eq("idcard", member.getIdcard()));

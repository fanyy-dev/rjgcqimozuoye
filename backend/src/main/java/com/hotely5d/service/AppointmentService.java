...
public class AppointmentService {
    ...
    public Page<Appointment> search(Page<Appointment> page, Appointment appointment) {
        Page<Appointment> resultPage = appointmentMapper.selectPage(page, getQueryWrapper(appointment));
        for (Appointment a : resultPage.getRecords()) {
    ...
    public List<Appointment> search(Appointment appointment) {
        return appointmentMapper.selectList(getQueryWrapper(appointment));
    }

    //条件构造器
    private QueryWrapper<Appointment> getQueryWrapper(Appointment appointment) {
        QueryWrapper<Appointment> queryWrapper = new QueryWrapper<>();
        if (null != appointment) {
            if (!StringUtils.isEmpty(appointment.getRemark())) {
                queryWrapper.like("remark", String.valueOf(appointment.getRemark()));
            }
            if (!StringUtils.isEmpty(appointment.getMemberId())) {
                queryWrapper.eq("member_id", String.valueOf(appointment.getMemberId()));
            }
            if (!StringUtils.isEmpty(appointment.getStatus())) {
                queryWrapper.eq("status", String.valueOf(appointment.getStatus()));
            }
